...
\centering
\begin{tikzpicture}
  % Nodes for Intellectons
  \node[circle, draw, fill=white, minimum size=1.5cm] (A) at (0,0) {$\intellecton_A$};
  \node[circle, draw, fill=white, minimum size=1.5cm] (B) at (4,0) {$\intellecton_B$};

  % Self-loops (mu)
  \draw[->, thick, loop above] (A) to[out=135,in=45, looseness=8] node[midway, above] {$\mu_A$} (A);
  \draw[->, thick, loop above] (B) to[out=135,in=45, looseness=8] node[midway, above] {$\mu_B$} (B);

  % Memory morphisms
  \draw[->, thick, bend left=20] (A) to node[midway, above] {$\mathcal{M}_A(B)$} (B);
  \draw[->, thick, bend right=20] (B) to node[midway, below] {$\mathcal{M}_B(A)$} (A);

  % Resonance morphisms (J_ij)
  \draw[->, dashed, thick] (A) to[out=30,in=150] node[midway, above] {$\mathcal{J}_{AB}$} (B);
  \draw[->, dashed, thick] (B) to[out=-30,in=-150] node[midway, below] {$\mathcal{J}_{BA}$} (A);

  % Adjoint Functors
  \draw[->, thick, blue] (-2,-2) -- node[midway, left] {$\Delta$} (A);
  \draw[->, thick, red] (A) -- node[midway, right] {$\Omega$} (-2,-2);
  \draw[->, thick, blue] (6,-2) -- node[midway, left] {$\Delta$} (B);
  \draw[->, thick, red] (B) -- node[midway, right] {$\Omega$} (6,-2);

  % Global Coherence Node
  \node[circle, draw, fill=green!20, minimum size=1cm] (Omega) at (2,-2) {$\Omega_t$};
  \draw[->, thick] (Omega) -- (A) node[midway, above left] {};
  \draw[->, thick] (Omega) -- (B) node[midway, above right] {};

\end{tikzpicture}
\caption{Recursive folds with adjoint functors $\Delta \dashv \Omega$ and global coherence $\Omega_t$. $\mu_A$, $\mu_B$ represent self-loops, $\mathcal{M}_A(B)$, $\mathcal{M}_B(A)$ denote memory morphisms, and $\mathcal{J}_{AB}$, $\mathcal{J}_{BA}$ are resonance morphisms between intellectons.}
\label{fig:lattice}
\end{figure}
